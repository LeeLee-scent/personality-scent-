...
function animateIntroPage() {
 ...
 logo.addEventListener('animationend', () => {
   // 2. "測一測" types out
   introTitleLeft.classList.add('typewriter');
   typeText(introTitleLeft, '測一測', 100, () => {
     introTitleLeft.classList.remove('typewriter');
     // 3. "屬於你的風格香" types out
...
function animateResultPage(resultData) {
 ...
 resultSubtitle.addEventListener('animationend', () => {
   ...
   resultTitle.addEventListener('animationend', () => {
     ...
     resultImageContainer.classList.add('animate-flip');
     ...
     resultImageContainer.addEventListener('animationend', () => {
       ...
       resultDesc.classList.add('animate-fade-in');
     }, { once: true });
   }, { once: true });
 }, { once: true });
}
...
function renderQuestion(){
 const q = questions.questions ? questions.questions : questions; // 處理 questions 可能的結構
...
nextBtn.addEventListener('click', ()=>{
 if (currentSelection) {
   scores = questions.questions ? updateScores(scores, currentSelection) : updateScoresDirectly(scores, currentSelection);
   current++;
   if(current < total) {
     renderQuestion();
   } else {
...
function updateScores(currentScores, type) {
 currentScores = currentScores || {};
 currentScores["woody"] = currentScores["woody"] || 0;
 currentScores["citrus"] = currentScores["citrus"] || 0;
 currentScores["floral"] = currentScores["floral"] || 0;
 currentScores["musk"] = currentScores["musk"] || 0;
 currentScores = {...currentScores, [type]: (currentScores || {}) [type] + 1};
 return currentScores;
}

function updateScoresDirectly(currentScores, type) {
 currentScores = currentScores || { woody: 0, citrus: 0, floral: 0, musk: 0 };
 currentScores = {...currentScores, [type]: currentScores [type] + 1};
 return currentScores;
}
